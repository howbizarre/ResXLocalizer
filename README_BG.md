...
- **Създаване на нов "master" `.resx` файл по всяко време** — бутонът стои постоянно горе в страничната лента, независимо дали проектът още няма `.resx` файлове, или вече има много.
...
3. Разширението автоматично претърсва цялата папка за `.resx` файлове:
   ...
   - **Ако все още няма нито един** — ще видиш само кратко съобщение, плюс същия бутон **"Create master .resx file"**, описан по-долу:

     ![Празно състояние — създаване на нов файл](media/screenshots/empty-state.png)

     Натискането му те превежда през два лесни стъпки: избираш папка, после въвеждаш име (например `Strings`) — разширението създава празен, валиден `.resx` файл (същата структура, която генерира и Visual Studio), готов за първите ти ключове.
...
Бутонът **"Create master .resx file"** горе е винаги там, независимо дали проектът вече има `.resx` файлове — ползвай го всеки път, когато трябва да започнеш ново семейство (напр. `Strings.resx` за нова функционалност), без да ти пречи дървото отдолу.
...
Над всяка таблица, горе вдясно, на нивото на заглавието й, стоят два бутона — зелен **Import** (иконка ↓) и син **Export** (иконка ↑):

![Бутони Export и Import над таблица с преводи](media/screenshots/table-view.png)

### Export

1. Натисни **Export**.
2. Избери **CSV** или **JSON** от малкия избор, който се появява.
3. Посочи къде да се запази файлът.

Експортираният файл има по един ред за всеки ключ, с колона за всеки език (колоната на базовия/default файла се казва просто `default`) — точната структура, която виждаш и в таблицата. Можеш да го отвориш в Excel, Google Sheets, или да го дадеш на преводаческа агенция, която дори не ползва VS Code.

### Import

1. Натисни **Import**.
2. Посочи `.csv` или `.json` файл — не е задължително да идва от Export на самото разширение; работи с всеки файл, който има колона/поле `Key` и съответстващи езикови колони.

Преди да се запише каквото и да е в `.resx` файловете, файлът се **валидира**:

- трябва да се разчете коректно (валиден CSV/JSON), с колона/поле `Key` на всеки ред;
- ако някой ред е неправилен или без ключ, импортът се **отхвърля напълно** — появява се диалог със списък от всички проблеми, без да е променено нищо на диска.

След успешна валидация:

- редове с **ключ, който още не съществува**, се добавят като нови преводи;
- редове с **ключ, който вече съществува**, презаписват съществуващата стойност за този език — всяко такова презаписване се записва в log;
- празните клетки се пропускат (празна клетка никога не изтрива съществуващ превод);
- колони, които не съвпадат с никой от езиковите файлове в тази таблица, просто се игнорират.

Ако по време на импорта са презаписани съществуващи стойности, автоматично се отваря **нов таб** веднага след приключването, в който са изредени всички презаписани ключове със старата и новата им стойност едно до друго, плюс евентуални предупреждения (напр. ключ, повторен вътре в самия импортиран файл) — за преглед какво точно се е променило, преди да се довериш на резултата:

![Таб с log на импорта, показващ презаписаните ключове със стари и нови стойности](media/screenshots/import-log.png)
...
- `npm run publish` — публикува текущата версия директно в Marketplace (`vsce publish`); изисква предварителен `vsce login <publisher>`.
